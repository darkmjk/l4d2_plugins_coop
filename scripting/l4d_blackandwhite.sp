...
public void OnPluginStart()
{
	...
	h_cvarNoticeType = CreateConVar("l4d_bandw_notice", "2", "0=turns notifications off, 1=notifies survivors, 2=notifies all, 3=notifies infected.", FCVAR_NOTIFY, true, 0.0, true, 3.0);
	h_cvarPrintType = CreateConVar("l4d_bandw_type", "0", "0=prints to chat, 1=displays hint box.", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	if(g_bL4D2Version)
	{
...
public void EventReviveSuccess(Event event, const char[] name, bool dontBroadcast) 
{
	...
	if(event.GetBool("lastlife"))
	{
		...
		else if(bandw_notice == 2) 
		{
			...
			{
				if(bandw_type == 1) PrintHintTextToAll("%N黑白了.", target);
				else PrintToChatAll("\x04[提示]\x03%N\x05黑白了.", target);
			}
			else
			{
				if(bandw_type == 1) PrintHintTextToAll("%N(%s)黑白了.", target, charName);
				else PrintToChatAll("\x04[提示]\x03%N\x01(\x03%s\x01)\x05黑白了.", target, charName);
			}
		}
		//print to infected
		else if(bandw_notice == 3)
		{
			for( int x = 1; x <= MaxClients; x++)
			{
				...
				if(IsFakeClient(target))
				{	
					if(bandw_type == 1) PrintHintText(x, "%N黑白了.", target);
					else PrintToChat(x, "\x04[提示]\x03%N\x05黑白了.", target);
				}
				else
				{
					if(bandw_type == 1) PrintHintText(x, "%N(%s)黑白了.", target, charName);
					else PrintToChat(x, "\x04[提示]\x03%N\x01(\x03%s\x01)\x05黑白了.", target, charName);	
				}
			}
		}
		//print to survivors
